### Homework #xx session questions

- Debug. Enable/disable globally. Enable during the development, disable before commit
- Login where confirmation is needed via email or phone application
- e2e courses for more advanced - skip JSCore

### Homework #6 session questions

- Git + branching
- e2e courses for more advanced - skip JSCore

### Homework #5 session questions

- I do not believe that DEVs are writing unit tests.
- Lecture for the errors handling or video for the AQA engineers
- in case of TS and specified parameter type - is it necessary to do typeof validations aka 'guard'?

### Homework #4 session questions

- emailLocator. does it make sense if variable type is Locator

### Homework #3 session questions

- XPath like //\*[@id="something"]

### Homework #2 session questions

- Global - QA dep in the company structure. Context: like an AI

- ';' at the end of code block. The best practice
- get page URL

- btn locator - do not use CSS is a best practice

```ts
locator('[class*="btn"]');
```

### Homework #1 session questions

- known defects (to be discussed later)
- handling tips, pop-ups, etc
- A library for string-to-number conversion and back
